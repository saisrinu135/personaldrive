...
def log_api_response(logger: logging.Logger, method: str, path: str, status_code: int,
    ...
    logger.info(
        f"API Response: {method} {path} - {status_code} ({response_time:.3f}s)", extra=extra_data)


def log_error(logger: logging.Logger, error: Exception, context: str = "", include_traceback: bool = True, **kwargs):
    ...
    logger.error(
        f"Error in {context}: {type(error).__name__}: {str(error)}", extra=extra_data, exc_info=include_traceback)
...
def log_storage_operation(logger: logging.Logger, operation: str, provider_id: str,
    ...
    logger.info(
        f"Storage Operation: {operation} - Provider: {provider_id}", extra=extra_data)
